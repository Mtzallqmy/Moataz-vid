...
class Media3Engine(
    ...
    override suspend fun prepareProject(graph: RenderGraph, mode: PreparationMode): MediaResult<PreparedProject> = runCatching {
        assertBound(graph)
        mapProduction(graph, preferProxy = mode == PreparationMode.PREVIEW)
        PreparedProject(graph.timelineRevision, BackendKind.MEDIA3, emptyList())
    }.fold(
        { MediaResult.Success(it) },
        { MediaResult.Failure(MediaEngineError.Media3UnsupportedOperation(it.message ?: "mapping")) },
    )
    ...
    override suspend fun preparePreview(graph: RenderGraph, surface: PreviewSurface): MediaResult<PreviewSession> = runCatching {
        ...
        player.prepare(id, mapProduction(graph, preferProxy = true), surface)
        PreviewSession(id, graph.timelineRevision, WysiwygLevel.EXACT)
    }.fold(
        { MediaResult.Success(it) },
        { MediaResult.Failure(MediaEngineError.Media3UnsupportedOperation(it.message ?: "preview")) },
    )

    override suspend fun updatePreview(sessionId: String, graph: RenderGraph): MediaResult<PreviewUpdate> = runCatching {
        assertBound(graph)
        player.replace(sessionId, mapProduction(graph, preferProxy = true))
        PreviewUpdate(graph.timelineRevision, rebuilt = true)
    }.fold(
        { MediaResult.Success(it) },
        { MediaResult.Failure(MediaEngineError.Media3UnsupportedOperation(it.message ?: "preview update")) },
    )
    ...
    override suspend fun seek(sessionId: String, position: TimeUs): MediaResult<Unit> = runCatching {
        ...
        { MediaResult.Failure(MediaEngineError.Media3UnsupportedOperation("seek")) },
    )
    ...
    override suspend fun export(request: ExportRequest): MediaResult<MediaJobHandle> {
        ...
        if (missing.isNotEmpty()) {
            return MediaResult.Failure(MediaEngineError.InvalidTimeline(missing.map { "Media3 binding missing ${it.name}" }))
        }
        ...
        val composition = try {
            mapProduction(request.graph, preferProxy = false)
        } catch (failure: Throwable) {
            return MediaResult.Failure(MediaEngineError.InvalidTimeline(listOf(failure.message ?: "video-use render preflight")))
        }
        val preflight = VideoUseMedia3Policy.inspect(composition)
        if (!preflight.passed) {
            return MediaResult.Failure(MediaEngineError.InvalidTimeline(preflight.issues.map { "video-use:$it" }))
        }
        return transformer.export(request.id, composition, request.outputUri, request.settings)
    }
    ...
    override suspend fun cancel(jobId: String): MediaResult<Unit> = runCatching { transformer.cancel(jobId) }
        .fold(
            { MediaResult.Success(Unit) },
            { MediaResult.Failure(MediaEngineError.Media3UnsupportedOperation("cancel")) },
        )
    ...
    private suspend fun mapProduction(graph: RenderGraph, preferProxy: Boolean): Media3CompositionSpec =
        VideoUseMedia3Policy.normalize(mapper.map(graph, preferProxy))
